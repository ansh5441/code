...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for j in range(300):
            patient.update()
            virPop[j] += patient.getTotalPop()
    for i in range(300):
        virPop[i] /= float(numTrials)
    pylab.plot(virPop)
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        mut = self.getMutProb()
        for drug in res.keys():
            if random.random() < mut:
                res[drug] = not res[drug]
        if self.getMaxBirthProb() * (1 - popDensity) > random.random():
            return ResistantVirus(self.getMaxBirthProb(), self.getClearProb(), res, mut)
        else:
            raise NoChildException
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for d in delay:
        virPop = []
        resistantVirPop = []
        for i in range(timeSteps + d):
            virPop.append(0)
            resistantVirPop.append(0)
        ...
        for i in range(numTrials):
            ...
            for j in range(d):
                patient.update()
                virPop[j] += patient.getTotalPop()
                resistantVirPop[j] += patient.getResistPop(['guttagonol'])
        ...
        for i in range(timeSteps + d):
            virPop[i] /= float(numTrials)
            resistantVirPop[i] /= float(numTrials)
        pylab.hist(virPop, label = 'viruses')
    
        pylab.hist(resistantVirPop, label = 'resistant viruses')
...
#simulationWithoutDrug(10, 90, 0.8, 0.1, 100)
